 ...
 for(int i=0;i<r->count_connectors;i++){
  ...
  if(c&&c->connection==DRM_MODE_CONNECTED&&c->encoder_id){
   ...
   if(e&&e->crtc_id){saved_crtc=drmModeGetCrtc(drmfd,e->crtc_id);
    if(saved_crtc&&saved_crtc->mode_valid&&saved_crtc->mode.hdisplay==RX3_PANEL_W&&saved_crtc->mode.vdisplay==RX3_PANEL_H){connector=c->connector_id;crtc=e->crtc_id;mode=saved_crtc->mode;}
